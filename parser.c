...
/*---------added function by yhmtrz------------*/
void expand_arguments_null(pc* command) {
    char **old_args = command->arguments;
    ...
    if ( old_args ) {
        memcpy(command->arguments, old_args, sizeof(char*)*(command->num_of_args-1));
        command->arguments[command->num_of_args-1] = NULL;
        free(old_args);
    }
}
/*-----------------------*/
...
input* parse(char *command_line) {
    ...
    while( (temp=(*ptr++)) != '\0' ) {
		...
		add_command_argument(result, buffer, buffer_index); // added line by yhmtrz
                phase = COMMAND_AFTER;
            }
            else
                buffer[buffer_index++] = temp;
